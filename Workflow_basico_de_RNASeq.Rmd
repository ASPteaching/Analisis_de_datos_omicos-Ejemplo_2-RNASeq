...
En la práctica se puede ejecutar este código una vez y después comentarlo o ponerlo a FALSE en la cabecera del bloque de Rmarkdown.

```{r instalPackages, eval=FALSE}
...
```{r}
knitr::include_graphics("images/mouse_exp.png")
```
...
url <- "https://www.ncbi.nlm.nih.gov/geo/download/?acc=GSE60450&format=file&file=GSE60450%5FLactation%2DGenewiseCounts%2Etxt%2Egz"
utils::download.file(url, destfile="GSE60450_Lactation-GenewiseCounts.txt.gz", mode="wb") 
R.utils::gunzip("GSE60450_Lactation-GenewiseCounts.txt.gz")
...
Para esta segunda representación utilizaremos la función `plotMDS` del paquete `limma`.
...
limma::plotMDS(logcounts_norm,col=col.status, main="Status", cex=0.7)
```
...
```{r} 
dotplot(ego, showCategory=9, font.size=6)
```
...
```{r eval=FALSE}
# En fech 2024-12-07 Esta funcion falla con R 4.4, por lo que se puede omitir su uso
goplot(ego)
```
